package org.innereye.gw.router;
 ...
 * {
 *     &#64;code
 *     Router<Class> router = new Router<Class>().GET("/articles", IndexHandler.class).GET("/articles/:id", ShowHandler.class).POST("/articles", CreateHandler.class).GET("/download/:*", DownloadHandler.class) // ":*"
 *                                                                                                                                                                                                               // must
 *                                                                                                                                                                                                               // be
 *                                                                                                                                                                                                               // the
 *                                                                                                                                                                                                               // last
 *                                                                                                                                                                                                               // token
 *             .GET_FIRST("/articles/new", NewHandler.class); // This will be
 *                                                            // matched first
 * }
 * </pre>
 *
 ...
 * <a
 * href="https://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol#Request_methods">OPTIONS</a>
 * or
 * <a
...
public class Router<T> {
     ...
     * <p>A path pattern can only point to one target. This method does nothing
     * if the pattern
    ...
    // --------------------------------------------------------------------------
    /**
     * If there's no match, returns the result with {@link #notFound(Object)
     * notFound}
     ...
     * <p>
     * For {@code OPTIONS *}, use {@link #allAllowedMethods()} instead of this
     * method.
     */
    ...
    public String uri(HttpMethod method, T target, Object... params) {
        ...
        // Fallback to anyMethodRouter if the router was not anyMethodRouter and
        // no path is found
     ...
     * Helper for toString.
     */
    private static <T> void aggregateRoutes(String method, Map<PathPattern, T> routes, List<String> accMethods,
            List<String> accPatterns, List<String> accTargets) {
